Healthcare Eligibility Pipeline - Configuration-Driven Data Ingestion
Overview
A scalable, medallion architecture data pipeline that ingests member eligibility files from multiple healthcare partners with varying file formats. The solution uses a configuration-driven approach to minimize code changes when onboarding new data sources.
Architecture: Bronze → Silver → Gold (Databricks/PySpark)
...
partner_id
INT
Unique partner identifier (Primary Key)
partner_code
STRING
Short code (e.g., 'acme', 'bettercare')
partner_name
STRING
Full partner name
file_delimiter
...
partner_id
INT
Foreign key to partner_config
source_column_name
STRING
Column name in partner file
target_field_name
STRING
...
external_id
STRING
Member identifier from partner
firstname
STRING
First name
lastname
...
validation_errors
STRING
Semicolon-separated error fields
created_date
TIMESTAMP
...
See Adding a New Partner section below.

Running the Pipeline
Option 1: Run Notebooks Sequentially
Execute notebooks in order:
Bronze.ipynb - Ingests raw data
python
# Widget parameters:
...
Silver.ipynb - Validates and cleanses
python
# Automatically processes all PENDING records
# No parameters needed


Gold.ipynb - Produces final output
python
...
Error Handling Flow
Silver layer validates all staging records
Invalid records → member_eligibility_rejected table
Valid records → marked validation_status = 'FINISHED'
Gold layer only processes valid records
Sample Error Output
text
